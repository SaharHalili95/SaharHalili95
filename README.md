# Hi there! 👋 I'm Sahar Halili

**Computer Science Graduate** | Full-Stack Developer | React, TypeScript, Python, FastAPI

🎓 B.Sc. Computer Science — Holon Institute of Technology (2024)
📍 Based in Israel | 🔭 Seeking junior/entry-level developer positions

---

## 🛠️ Tech Stack

**Languages:** Python, JavaScript, TypeScript, Java, C++
**Frontend:** React, HTML5, CSS3
**Backend:** FastAPI, Node.js, MongoDB
**Tools:** Git, Vite, GitHub Pages

---

## 💻 Featured Projects

### [Budget Balance Game](https://saharhalili95.github.io/budget-balance-game/) — React + TypeScript
Interactive financial education game with 5 levels, 8 achievements, monthly challenges, and a financial advisor report system. Bilingual (EN/HE).

### [Price Comparison Platform](https://saharhalili95.github.io/price-comparison-platform/) — FastAPI + React
Full-stack price comparison app with 3,000+ products across 8 categories. Built for the Israeli market.

### [Interview Prep Tracker](https://saharhalili95.github.io/interview-prep-tracker/) — FastAPI + MongoDB + React
Full-stack app for managing interview questions with progress tracking, categorization, and difficulty levels.

### [Portfolio](https://saharhalili95.github.io/html-portfolio/) — React + TypeScript
Personal portfolio showcasing my projects and skills.

---

## 📊 GitHub Stats

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=SaharHalili95&theme=radical&hide_border=true&include_all_commits=false&count_private=false" alt="GitHub Stats"/>
  <br/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=SaharHalili95&theme=radical&hide_border=true&include_all_commits=false&count_private=false&layout=compact" alt="Top Languages"/>
</div>

---

## 🌐 Connect with Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/sahar-halili-36ba38300/)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:sahar_halili@icloud.com)
[![Portfolio](https://img.shields.io/badge/Portfolio-FF5722?style=for-the-badge&logo=google-chrome&logoColor=white)](https://saharhalili95.github.io/html-portfolio/)
